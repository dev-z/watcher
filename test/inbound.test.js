...
describe('testing /inbound/sms', function () {
  // --- HOOKS ----------------------------------------------------------------------- //
  ...
  describe('/POST /inbound/sms', function () {
    // Valid Requests ------------------------------------------------------------------ //
    it('it should allow inbound sms when all params are correct.', function (done) {
      chai.request(server)
    ...
    it('it should NOT allow if the ‘to’ parameter is not present in the phone_number table for this specific account.', function (done) {
      chai.request(server)
        .post('/api/v1/inbound/sms')
        .set('authorization', global.testData.user1.basicAuth)
        .send({
        ...
        .end(function (err, res) {
          ...
          res.body.should.be.an('object');
          res.body.should.have.property('error')
            .that.is.equal('to parameter not found');
          done();
        });
    });
  });

  describe('/GET /api/v1/inbound/sms', function () {
    it('it should not allow GET method.', function (done) {
      chai.request(server)
        .get('/api/v1/inbound/sms')
        .end(function (err, res) {
  ...
  describe('/PUT /api/v1/inbound/sms', function () {
    it('it should not allow PUT method.', function (done) {
      chai.request(server)
        .put('/api/v1/inbound/sms')
        .end(function (err, res) {
          res.should.have.status(405);
          done();
        });
